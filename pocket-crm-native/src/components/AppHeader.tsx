...
import { Bell, CalendarCheck, Sparkles } from 'lucide-react-native';
import { useCrm } from '../store';
import { c, r } from '../theme';
import { Avatar } from './ui';

export function AppHeader() {
  const { userProfile, unreadCount, setNotificationsOpen, goToOpeningPage } = useCrm();
  ...
  return (
    <View style={{ paddingTop: insets.top, backgroundColor: c.white, borderBottomWidth: 1, borderBottomColor: c.slate100 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 10 }}>
        <Pressable onPress={goToOpeningPage} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          <View>
            ...
            <Text style={{ fontSize: 10, fontWeight: '600', color: c.indigo600 }}>← Start Page</Text>
          </View>
        </Pressable>

        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <Pressable onPress={goToOpeningPage} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: r.full, backgroundColor: c.slate100 }}>
            <CalendarCheck size={14} color={c.indigo600} />
            <Text style={{ fontSize: 11, fontWeight: '600', color: c.slate600 }}>Opening Page</Text>
          </Pressable>
