...
import { CalendarDays, FileDown, Pencil, Plus, Target, X } from 'lucide-react-native';
...
export function ProfileView() {
  ...
  const startEdit = () => { setForm({ ...userProfile, socialLinks: { ...userProfile.socialLinks }, networkingGoals: [...userProfile.networkingGoals] }); setNewGoal(''); setEditing(true); };
  const save = () => { if (form.name.trim()) updateProfile(form); setEditing(false); };
  ...
  const addGoal = () => { const g = newGoal.trim(); if (g && !form.networkingGoals.includes(g)) set({ networkingGoals: [...form.networkingGoals, g] }); setNewGoal(''); };
      ...
      {editing ? (
        <Card style={{ padding: 20, borderColor: c.indigo500 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <Avatar name={form.name} color={form.avatarColor} size={40} radius={12} />
          </View>
          ...
          <Text style={{ fontSize: 12, fontWeight: '700', color: c.slate900, marginBottom: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: c.slate100 }}>Networking goals</Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
            {form.networkingGoals.map((g) => (
              <View key={g} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: c.indigo50, borderWidth: 1, borderColor: c.indigo200, borderRadius: r.md, paddingLeft: 10, paddingRight: 6, paddingVertical: 5 }}>
                ...
                <Pressable onPress={() => set({ networkingGoals: form.networkingGoals.filter((x) => x !== g) })} hitSlop={6}>
        ...
        <>
          ...
          <Card>
            ...
            {userProfile.networkingGoals.length === 0 ? (
              <Text style={{ fontSize: 12, color: c.slate400 }}>No goals yet. Tap Edit above to add some.</Text>
            ) : (
