...
export function AddContactCard() {
  const { setScanOpen, setSpeakOpen } = useCrm();
  ...
  return (
    <View style={{ backgroundColor: c.accent, borderRadius: r.xxl, padding: 20, gap: 16 }}>
      ...
      <View style={{ flexDirection: 'row', gap: 10 }}>
        <Pressable onPress={() => setSpeakOpen(true)} style={({ pressed }) => ({ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 13, borderRadius: r.full, backgroundColor: c.text, opacity: pressed ? 0.85 : 1 })}>
